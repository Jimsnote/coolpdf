...
- 8 语言字典类型同构（`Record<Locale, Dictionary>` 编译期强制）；新工具允许英文占位先行、翻译随后补全（heic-to-pdf、sign-pdf 均已按此流程完成：英文占位上线 → 7 语言翻译补齐），当前零遗留英文
...
### 6.1 新语言扩展决策框架（2026-07-28 定，暂不扩）

**结论：沙盒期不加新语言**——瓶颈是域名权重而非语言覆盖；每加 1 种语言，未来所有新工具/教程/文案的翻译成本永久 +1，须用数据换。4 周后按 GSC「国家」维度数据执行三筛子评估：

1. **需求证据**：该国展示量进前 15 且无对应语言版本
2. **Google 市占率 >50%**（Naver 主导的韩国、Yandex 主导的俄罗斯天然打折）
3. **变现或竞争价值**：AdSense RPM 中高，或长尾竞争弱（小语种易上首页，如 ja 首周即有词排第 3）

- **候选排序**（当前预判）：韩语 ≈ 越南语 > 波兰语 > 土耳其语；印尼语量大钱少往后排。注：首周唯一点击来自越南
- **明确不加**：荷兰语/北欧诸语（用户搜工具用英文）、印地语（GSC 显示印度全是英文查询）、俄语（AdSense 已停服俄罗斯 + Yandex 主导，流量有收入无）、阿拉伯语（RTL 是全站布局镜像工程，非翻译活）
- **阶段原则**：既有语言的内容深度 > 新语言的广度
