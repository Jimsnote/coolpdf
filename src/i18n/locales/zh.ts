export const zh = {
  ...
  tools: {
    ...
    'heic-to-pdf': {
      name: 'HEIC 转 PDF',
      description: '将 iPhone HEIC 照片转换为整洁、便于分享的 PDF 文档。',
    },
    'sign-pdf': {
      name: 'PDF 签名',
      description: '绘制签名，并将其摆放到 PDF 文档的任意位置。',
    },
    'protect-pdf': {
      name: '保护 PDF',
  ...
  toolUi: {
    ...
    dropHeic: '将 HEIC 或 HEIF 照片拖到此处，或点击选择文件',
    moveUp: '上移',
    moveDown: '下移',
    remove: '移除',
    processing: '处理中…',
  ...
  toolPages: {
    ...
    'heic-to-pdf': {
      metaTitle: 'HEIC 转 PDF 转换器——免费、本地、无需上传 | CoolPDF',
      metaDescription:
        '在浏览器中将 iPhone 的 HEIC 照片转换为整洁的 A4 PDF。适应、铺满或原始尺寸——照片永不离开您的设备。免费，无需注册。',
      heading: 'HEIC 转 PDF',
      intro: '将最多 20 张 iPhone HEIC 照片转换为一个 A4 PDF。HEIC 解码与 PDF 组装都完全在您的设备上进行。',
      button: '转换为 PDF',
      orientationLabel: '页面方向',
      orientationAuto: '自动（跟随第一张照片）',
      orientationPortrait: '纵向',
      orientationLandscape: '横向',
      fitLabel: '图片摆放方式',
      fitFit: '适应——完整显示照片，保留边距',
      fitFill: '铺满——铺满页面，裁切边缘',
      fitOriginal: '原始——原始尺寸，居中放置',
      decodingProgress: '正在解码第 {current} 张照片，共 {total} 张…',
      decodeError:
        '无法解码“{name}”——它可能使用了此解码器不支持的 HEIC 变体。请尝试在 iPhone 上将该照片分享为 JPEG 后再转换。',
      stepsHeading: '如何将 HEIC 照片转换为 PDF',
      steps: [
        '将最多 20 张 HEIC 或 HEIF 照片拖到上传区域，或点击选择文件。',
        '选择页面方向：自动模式跟随第一张照片的宽高比，或强制纵向、横向。',
        '选择照片摆放方式——适应留白、铺满页面或保持原始尺寸——然后点击“转换为 PDF”。',
        '下载您的 PDF，每张照片占据一个 A4 页面。',
      ],
      privacyHeading: 'iPhone 照片转 PDF，连一个像素都不必上传',
      privacyText:
        '照片是我们大多数人拥有的最私密的文件——家人、证件、白板、收据。典型的 HEIC 转换器会要求您把每张照片上传到服务器，在那里转换，然后只能相信它事后会被删除。CoolPDF 在您的设备上完成全部工作：libheif 解码器的 WebAssembly 版本就在您的浏览器标签页中以后台线程运行，把每张 HEIC 照片解码为 JPEG，再打包成一个 A4 PDF——全程本地完成。您的照片从不接触网络，因此不可能被记录、泄露或进入某个数据集。页面加载完成后它甚至可以离线使用：打开飞行模式，照常转换。',
      faqHeading: '常见问题',
      faq: [
        {
          question: '什么是 HEIC？为什么要转换为 PDF？',
          answer:
            'HEIC 是自 iOS 11 以来 iPhone 的默认照片格式。它让文件体积更小，但许多应用、网站和 Windows 电脑都无法打开它。PDF 则在哪里都能打开——还能把多张照片打包成一个便于分享或打印的文档。',
        },
        {
          question: '为什么我的某个 HEIC 文件转换失败？',
          answer:
            'HEIF 是一种容器格式，可以封装不同的编解码器和特性，没有任何浏览器解码器能支持所有变体。用 iPhone 以标准 HEVC 格式拍摄的照片可以可靠转换。如果某个文件转换失败，请在 iPhone 上打开它并分享为 JPEG（或按「相机 → 格式 → 兼容性最佳」设置相机），然后转换得到的文件。',
        },
        {
          question: '照片的画质和方向会保留吗？',
          answer:
            '会。每张照片都以全分辨率解码，并写成高质量 JPEG（92% 画质），存储的旋转信息会自动应用。生成的 PDF 直接嵌入这些图片，不做任何进一步压缩。',
        },
      ],
    },
    'sign-pdf': {
      metaTitle: 'PDF 签名工具——免费、本地、无需上传 | CoolPDF',
      metaDescription:
        '在浏览器中绘制签名，摆放到 PDF 任意位置，下载签好的文档——文件永不离开您的设备。免费，无需注册。',
      heading: 'PDF 签名',
      intro:
        '用鼠标、触控板或手指绘制签名——或上传签名照片——然后摆放到 PDF 任意页面的任意位置，下载签好的文档。一切处理都在您的设备本地完成。',
      button: '签名并下载',
      padLabel: '您的签名',
      padHint: '在此绘制您的签名',
      clearPad: '清除',
      addToPage: '添加到当前页面',
      placementHint: '拖动签名移动位置——拖动角上的圆点调整大小。',
      ...
      uploadChange: '换一张图片',
      uploadHint:
        '签名的照片或扫描件效果最好——纯白纸张上的深色字迹。我们会在您的浏览器中直接为您去除背景。',
      uploadRemoveBg: '去除白色背景',
      uploadStrength: '去除强度',
      uploadEmpty: '去除背景后没有剩余内容——请尝试降低强度。',
      uploadError:
        '无法读取该图片。请尝试 PNG 或 JPG 格式的签名照片（iPhone 拍摄的 HEIC 也可以）。',
      stepsHeading: '如何为 PDF 签名',
      steps: [
        '将要签署的 PDF 拖到上传区域，或点击选择文件——文件在本地打开，绝不会发送到服务器。',
        '在签名板中绘制签名——或切换到“上传”使用签名照片——然后点击“添加到当前页面”。',
        '把签名拖到合适的位置，拖动角上的圆点调整大小。切换页面即可在多处签名。',
        '点击“签名并下载”，获取签好名的 PDF。',
      ],
      privacyHeading: '签署合同，无需上传',
      privacyText:
        '需要签署的文档，往往是您经手的最敏感的文件：劳动合同、保密协议（NDA）、租约、发票。大多数在线签名工具都要求您把文档——连同您的签名——上传到它们的服务器，两者一起留在您无法审计的队列和存储里。CoolPDF 反其道而行之：文档由您自己的浏览器渲染，您绘制或选择的签名也永不离开您的设备。它只存在于这个标签页的内存中，在本地盖进 PDF，关闭页面的瞬间即消失。没有账号，没有上传，除了您下载的那份文档之外，您的签名在任何地方都不留副本。',
      faqHeading: '常见问题',
      faq: [
        {
          question: '手绘签名具有法律效力吗？',
          answer:
            '在许多司法辖区，日常文档是可以的：美国 ESIGN 法案和欧盟 eIDAS 法规都承认简单电子签名对大多数合同有效。但有些文档——遗嘱、经公证的契约、某些政府表格——要求特定形式或合格签名。如有疑问，请核实您所签文档的具体要求。',
        },
        {
          question: '可以用签名照片代替手绘吗？',
          answer:
            '可以。切换到“上传”标签页，选择签名的照片或扫描件——纯白纸张上的深色字迹效果最干净。白色背景会在您的浏览器中自动去除，签名因此能自然融入文档。没有任何内容被上传到任何地方。',
        },
        {
          question: '可以在多个位置签名吗？',
          answer:
            '可以。您可以在任意页面添加任意数量的签名——每个签名都能独立移动和调整大小。下载时它们会全部盖进同一个 PDF。',
        },
        {
          question: '你们会保存我的签名吗？',
          answer:
            '不会。您的笔迹只存在于当前浏览器标签页的内存中，关闭或刷新页面即消失。它绝不会被上传、保存或发送到任何地方。',
        },
      ],
    },
    'organize-pdf': {
    ...
    'reorder-pages': {
      ...
      metaDescription:
        '拖拽缩略图即可重新排列 PDF 页面顺序，实时预览每一页，几秒完成重排。100% 浏览器本地处理——无需上传、无需注册、免费无限制。',
      heading: '调整 PDF 页面顺序',
      intro:
    ...
    'qr-code': {
      metaTitle: '二维码生成器 - 免费、本地、无需注册 | CoolPDF',
      metaDescription:
        '在浏览器中为链接、文本、Wi-Fi 等生成二维码——随输入即时生成，完全在本地完成。无需上传、无需注册、永久免费。可下载 PNG 或 SVG。',
      heading: '二维码生成器',
      intro:
        '输入或粘贴任意链接或文本，二维码会随着您的输入即时出现。生成完全在您自己的设备上进行——没有任何内容被发送出去。',
      inputLabel: '内容',
      ...
      downloadPng: '下载 PNG',
      downloadSvg: '下载 SVG',
      emptyHint: '开始输入后，您的二维码会立即显示在这里。',
      errorTooLong:
        '内容太长，超出了一个二维码的容量。请缩短内容，或选择更低的纠错级别。',
      privacyNoteInput:
        '您的文本绝不会离开浏览器——二维码完全在本地生成。',
      stepsHeading: '如何生成二维码',
      steps: [
        '在内容框中输入或粘贴链接、一段文本、Wi-Fi 凭据或其他任何内容。',
        '二维码会随着您的输入即时出现——如有需要，可选择尺寸和纠错级别。',
        '较高的纠错级别能让二维码在印得较小或略有磨损时仍可扫描；较低的级别可容纳更多数据。',
        '下载清晰的 PNG 图片，或下载可放大到任意印刷尺寸的 SVG。',
      ],
      privacyHeading: '一个看不到您编码内容的二维码生成器',
      privacyText:
        '大多数二维码生成器会把您的文本发送到服务器，再返回一张图片——这意味着服务器能看到人们编码的每一个 Wi-Fi 密码、私密链接和电话号码。CoolPDF 反其道而行之：生成器代码来到您的浏览器，您的文本绝不离开这个标签页。您可以一边输入一边打开网络面板观察——没有任何内容被发送。这使得它可以安全地用于凭据、内部网址，以及任何您不愿让他人看到的内容。',
      faqHeading: '常见问题',
      faq: [
        {
          question: '二维码会过期吗？',
          answer:
            '永远不会。二维码只是您内容的一张普通图片——没有账号、没有跳转服务、没有有效期。下载之后，它会一直有效。',
        },
        {
          question: '生成的二维码可以商用吗？',
          answer:
            '可以。您生成的一切都归您所有——可用于包装、海报、名片或客户项目，完全免费，也无需署名。',
        },
        {
          question: '我该选择哪个纠错级别？',
          answer:
            '中等级别（默认）适合大多数用途。印刷尺寸较小或预计会有磨损时选“高”——二维码最多可承受 30% 的损坏；需要在一个码中容纳更多数据时选“低”。',
        },
        {
          question: '我的文本会被上传到任何地方吗？',
          answer:
            '不会。二维码完全在您的浏览器中用 JavaScript 生成。页面加载后您甚至可以断开网络——它依然可用。',
        },
      ],
    },
    'ocr-pdf': {
      metaTitle: 'OCR 文字识别 - 本地让扫描件 PDF 可搜索 | CoolPDF',
      metaDescription:
        '在浏览器中用 OCR 为扫描件 PDF 添加真正的文字层，使其可搜索、可选择、可复制。支持英文识别。无需上传、无需注册、永久免费。',
      heading: 'OCR 文字识别',
      intro:
      ...
      engineLoading:
        '正在加载 OCR 引擎——首次运行需下载约 7 MB，之后会缓存在您的设备上…',
      processingPages: '正在识别第 {current} 页，共 {total} 页…',
      downloadPdf: '下载可搜索的 PDF',
      downloadText: '下载文本（.txt）',
      stepsHeading: '如何对 PDF 进行 OCR 识别',
      steps: [
      ...
      privacyHeading: 'OCR 识别机密扫描件，无需发送到任何地方',
      privacyText:
      ...
      faq: [
        ...
        {
          question: '已经有文字层的 PDF 还需要 OCR 吗？',
          answer:
